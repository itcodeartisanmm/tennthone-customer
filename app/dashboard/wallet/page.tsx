    ...
    return (
        <div>
            <div className="text-center p-20">
                <h1 className="text-6xl font-bold"> Hello , <span className="text-primary"> {user?.name} </span></h1>
                <p className="text-gray-500"> Welcome to your wallet </p>
            </div>
            {dashboardData && (
                <div className="flex flex-col gap-4">
                    <Balance dashboardData={dashboardData} setDashboardData={setDashboardData} />
                    {dashboardData?.recentOrders && <RecentIncome recentIncome={dashboardData?.recentOrders} />}
                </div>
            )}
        </div>
    );
